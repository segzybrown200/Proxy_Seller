...
const Address = () => {
  ...
  setCoords({ latitude: loc.coords.latitude, longitude: loc.coords.longitude });
      ...
      if (rev && rev.length > 0) {
        ...
        const finalAddr = addr || `${loc.coords.latitude.toFixed(6)}, ${loc.coords.longitude.toFixed(6)}`;
    ...
    <>
      ...
      "
          ...
          <View className="px-5 mt-[50px]">
            <Text className="text-xl font-RalewayMedium text-gray-500">Current location</Text>
            <View className="mt-4 flex-row items-center gap-3">
              ...
              <View style={{ flex: 1 }}>
                <Text className="text-lg font-NunitoMedium">{address || (error ? error : 'Location not available')}</Text>
                <Text className="text-lg font-NunitoLight text-gray-500">Tap the icon to refresh location</Text>
              </View>
            </View>
